...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement('
            ...
            FOR EACH ROW
            BEGIN
                IF NEW.role_id = 4 AND (SELECT COUNT(*) FROM finance_users WHERE role_id = 4) >= 1 THEN
                    SIGNAL SQLSTATE "45000" SET MESSAGE_TEXT = "Only one finance_users can have role_id of 4.";
                END IF;
            END;
        ');
        ...
        DB::statement('
            ...
            FOR EACH ROW
            BEGIN
                IF NEW.role_id = 4 AND (SELECT COUNT(*) FROM finance_users WHERE role_id = 4 AND id != NEW.id) >= 1 THEN
                    SIGNAL SQLSTATE "45000" SET MESSAGE_TEXT = "Only one finance_users can have role_id of 4.";
                END IF;
            END;
        ');
